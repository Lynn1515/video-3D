...
class ViewCrafter:
    def __init__(self, opts, gradio = False):
        ...
        if not gradio:
            if os.path.isfile(self.opts.image_dir):
                images_dict, self.img_ori = self.load_initial_images(self.opts.image_dir)
                images = []
                for image_ in images_dict:
                    images.append((image_["img"][0] + 1.)/2)
                self.images =  load_and_preprocess_images(images).to(self.device)#[1,3,294,518]
                self.run_vggt(images=self.images[None], direct_pred_points=False)
            elif os.path.isdir(self.opts.image_dir):
                self.images, self.img_ori = self.load_initial_dir(self.opts.image_dir)
                self.run_vggt(input_images=self.images)
            else:
    ...
    def setup_vggt(self):
        ...
        checkpoint_path = "/home/cx_wchn/lnwang/ViewCrafter/checkpoints/vggt_model.pt" #self.opts.vggt_ckpt
        ...
        self.model.eval()
        print(f"✅ VGGT model loaded from {checkpoint_path}")

    def run_vggt(self, images, clean_pc=False, direct_pred_points=False):
        """使用 VGGT 推理图像，输出相机参数、深度图和点云"""
        # 输入预处理
        # images = []
        # for image in input_images:
        #     images.append(image['img'])
        # images = torch.stack(images, dim=0)
        # images = images.to(self.device)#load_and_preprocess_images(input_images).to(self.device)[None]

        with torch.no_grad():
            with torch.cuda.amp.autocast():
                if direct_pred_points:
                    ...
                    point_map = point_map.squeeze(0).detach().cpu().numpy()
                else:
                # 提取聚合特征
                    ...
                    point_conf = depth_conf.squeeze(0)
    ...
    def load_and_preprocess_image(self,
        ...
        """
        读取并处理图像：
        - 若图像高度 > 宽度，则中心裁剪为横屏 16:9 比例；
        - 最终输出 [3, target_height, target_width]，范围 [0,1]。

        Args:
            image_path (str): 图像路径
            target_height (int): 输出高度 (默认 288)
            target_width (int): 输出宽度 (默认 512)

        Returns:
            torch.Tensor: 处理后的图像张量 [3, H, W]
        """
        ...
        # 若原图偏“竖”，则从中间截取宽度方向更多区域，使之接近16:9
    ...
    def nvs_single_view(self, gradio=False):
        ...
        #     "point_conf": point_conf,
        # }
        extrinsic = self.scene["extrinsic"].squeeze(0)  # (N, 3, 4)
        intrinsic = self.scene["intrinsic"].squeeze(0)  # (N, 3, 3)
        depth = self.scene["depth"].squeeze(0)          # (N, H, W, 1)
        points = torch.from_numpy(self.scene["points"]).to(self.device).float()     # (N, H, W, 3)
        #c2ws = self.scene["extrinsic"]
        N = extrinsic.shape[0]
        c2ws_test = closed_form_inverse_se3(extrinsic) 
        c2ws = []
        for i in range(N):
            ext_3x4 = extrinsic[i]                       # (3, 4)
            ext_4x4 = torch.eye(4, device=ext_3x4.device, dtype=ext_3x4.dtype)
            ext_4x4[:3, :] = ext_3x4
            c2ws.append(ext_4x4)
        c2ws = torch.stack(c2ws, dim=0).detach() # (N, 4, 4)
        if (c2ws == c2ws_test).all():
            print('[DEBUG] equal')
        # 取 fx, fy，然后平均为单个标量焦距，保持 shape = (N, 1)
        focals = intrinsic[:, [0, 1], [0, 1]].mean(dim=1, keepdim=True).detach()
        principal_points = intrinsic[:, [0, 1], [2, 2]].detach()#[1:]  # cx, cy
        pcd = [p.detach() for p in points]              # list of (H, W, 3)
        depth_maps = [d.squeeze(-1).detach() for d in depth]  # list of (H, W)

        H, W = depth_maps[0].shape
        depth_center = depth_maps[-1][H // 2, W // 2]
        radius = depth_center * self.opts.center_scale
        c2ws_copy = c2ws
        # Compute scene center and recenter
        scene_center = points.reshape(-1,3).mean(dim=0)                       # 计算场景中心
        points_centered = points.reshape(-1,3) - scene_center   
        c2ws[:, :3, 3] -= scene_center
        
        c2ws_test2, pcd =  world_point_to_obj(poses=c2ws_test, points=torch.stack(pcd), k=-1, r=radius, elevation=self.opts.elevation, device=self.device)
        #points_centered = pcd.reshape(-1, 3)
        if (c2ws == c2ws_test2).all():
            print('[DEBUG] equal')
        imgs = self.scene["images"].permute(0, 2, 3, 1)
        ...
        if self.opts.mode == 'single_view_nbv':
            ...
            positions, idx_ = sample_ring_candidates(R_ref, T_ref, num_generate=3000, desired_count=100, radius=radius, min_angle_deg=20.0, max_angle_deg=30.0, min_sep_deg=6.0)
            ...
            pts = points_centered #torch.cat([p for p in [points_centered[-1]]], dim=0).view(-1, 3).to(self.device)
            col = imgs.reshape(-1, 3).to(self.device)
        ...
        render_results, viewmask = self.run_render([points_centered], [imgs[-1]], masks, H, W, camera_traj, num_views)
        ...
        save_pointcloud_with_normals([imgs[-1]], [points_centered], msk=None, save_path=os.path.join(self.opts.save_dir,'pcd0.ply') , mask_pc=False, reduce_pc=False)
    ...
    def nvs_sparse_view(self,iter):
        ...
        intrinsic = self.scene["intrinsic"].squeeze(0)  # (N, 3, 3)
        depth = self.scene["depth"].squeeze(0)          # (N, H, W, 1)
        depth_conf = self.scene["point_conf"].squeeze(0) # (N, H, W)
        points = torch.from_numpy(self.scene["points"]).to(self.device).float()     # (N, H, W, 3)
        #c2ws = self.scene["extrinsic"]
        N = extrinsic.shape[0]
        #c2ws = closed_form_inverse_se3(extrinsic) 
        c2ws = []
        for i in range(N):
            ext_3x4 = extrinsic[i]                       # (3, 4)
            ext_4x4 = torch.eye(4, device=ext_3x4.device, dtype=ext_3x4.dtype)
        ...
        c2ws = torch.stack(c2ws, dim=0).detach() # (N, 4, 4)
        # 取 fx, fy，然后平均为单个标量焦距，保持 shape = (N, 1)
        focals = intrinsic[:, [0, 1], [0, 1]].mean(dim=1, keepdim=True).detach()#5,1
        principal_points = intrinsic[:, [0, 1], [2, 2]].detach()#[1:]  # cx, cy  5,2
        pcd = [p.detach() for p in points]              # list of (H, W, 3)
        depth_maps = [d.squeeze(-1).detach() for d in depth]  # list of (H, W)
        H, W = depth_maps[0].shape
        depth_center = depth_maps[-1][H // 2, W // 2]
        ...
        ## masks for cleaner point cloud
                # 最大值和最小值
        max_val = depth_conf.max()
        min_val = depth_conf.min()
        print(f"Max: {max_val}, Min: {min_val}")
        # 平均值和标准差
        mean_val = depth_conf.mean()
        std_val = depth_conf.std()
        print(f"Mean: {mean_val}, Std: {std_val}")
        # 查看大概分布：可以用直方图
        hist = torch.histc(depth_conf, bins=20, min=min_val, max=max_val)  # 20个区间
        print(f"Histogram: {hist}")
        # min_conf_thr = 0.5 #float(self.scene.conf_trf(torch.tensor(self.opts.min_conf_thr)))
        # bgs_mask = [dpt > min_conf_thr for dpt in depth_conf]
        masks = 0#bgs_mask
        ...
        if self.opts.mode == 'single_view_ref_iterative':
            ...
            print(radius, c2ws[0,2,3])
            positions, belong_idx = sample_ring_candidates(R_ref_first, T_ref_first, num_generate=(iter + 1) * 3000, desired_count=300, radius=radius, min_angle_deg=20.0, max_angle_deg=30.0, min_sep_deg=6.0)
            Rs, Ts = look_at_R_from_positions(positions)
            print(belong_idx)
            ...
            depth_conf = depth_conf.reshape(-1)
            init_threshold_val = torch.quantile(depth_conf, 0.25)
            init_conf_mask = (depth_conf >= init_threshold_val) & (depth_conf > 0.1)
            pts = points_centered[init_conf_mask]
            col = imgs.reshape(-1, 3)[init_conf_mask]
            #torch.cat([p[m] for p, m in zip(pcd, masks)]).to(self.device)
            ...
            render_results, viewmask = self.run_render(points_centered, imgs.reshape(-1, 3), None, H, W, camera_traj, num_views)
            render_results = F.interpolate(render_results.permute(0,3,1,2), size=(576, 1024), mode='bilinear', align_corners=False).permute(0,2,3,1)
            #render_results[0] = (self.images[-1]['img_ori'].squeeze(0).permute(1,2,0)+1.)/2.
            render_results[0] = self.img_ori
        ...
        save_pointcloud_with_normals(imgs.reshape(-1, 3), points_centered, msk=None, save_path=os.path.join(self.opts.save_dir, f'pcd{iter}.ply') , mask_pc=False, reduce_pc=False)
        diffusion_results = self.run_diffusion(render_results)
    ...
    def single_view_best_select_iterative(self):
        ...
        for itr in range(0, 5):
            if itr == 0:
                self.images = [self.images] #去掉后一份copy（3，294，518）
                diffusion_results_itr = self.nvs_single_view()
                # diffusion_results_itr = torch.randn([25, 576, 1024, 3]).to(self.device)
                diffusion_results_itr = diffusion_results_itr.permute(0,3,1,2)
                all_results.append(diffusion_results_itr)
                ...
                for i in range(0+sample_rate, diffusion_results_itr.shape[0], sample_rate):
                    img_resized = F.interpolate(diffusion_results_itr[i:i+1,...].to(torch.float32), size=(294, 518), mode="bicubic", align_corners=False)
                    self.images.append(img_resized)
                    idx += 1
                images_group = torch.stack(self.images, dim=1)
                self.run_vggt(images_group,direct_pred_points=False)
                diffusion_results_itr = self.nvs_sparse_view(itr)
                # diffusion_results_itr = torch.randn([25, 576, 1024, 3]).to(self.device)
                diffusion_results_itr = diffusion_results_itr.permute(0,3,1,2)
                all_results.append(diffusion_results_itr)
